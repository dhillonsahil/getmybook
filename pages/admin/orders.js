...
export default function Viewproducts({ order }) {
    ...
    const handleChange = (index, field, value, delivery ,status) => {
        // Update state for the specific item at the given index
        setItemStates((prevStates) => {
            const updatedStates = [...prevStates];
            updatedStates[index] = { ...updatedStates[index], [field]: value,delivery:delivery,status:status};
                    ...
                    {orders.length > 0 &&
                        orders.map((item, index) => (
                            <div key={item._id} className="max-w-sm rounded overflow-hidden shadow-lg">
                             ...
                             <p> Email : {JSON.parse(item.address).email} ,
                             Mobile : {JSON.parse(item.address).phone}</p>
                             <p>
                                Address : {JSON.parse(item.address).pincode} , {JSON.parse(item.address).city} , {JSON.parse(item.address).state}
                             </p>
                                    </div>
                                    <div className="px-6 pt-4 pb-2">
                             <span className={`inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold  ${item.delivery=="unshipped"?'bg-yellow-200':'bg-green-500'} text-gray-700 mr-2 mb-2`}>Delivery : {item.delivery}</span>
                             <span className={`inline-block ${item.status=="PAID"?'bg-green-500':'bg-yellow-200'} bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2`}>Payment Status : {item.status}</span>
                                ...
                                <form onSubmit={(e) => handleSubmit(e, item.orderId)}>
                                    <div className="grid gap-6 m-6 md:grid-cols-2">
                                        <div>
                                            ...
                                            <input
                                                type="text"
                                                onChange={(e) => handleChange(index, 'Delivery', e.target.value,item.delivery,item.status)}
                                                value={itemStates[index]?.Delivery === 0 ? '' : itemStates[index]?.Delivery}
                                        ...
                                        <div>
                                            ...
                                            <input
                                                ...
                                                value={itemStates[index]?.Payment === 0 ? '' : itemStates[index]?.Payment}
                                                className="bg-gray-50 border border-gray-300 text-black text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                                placeholder="Enter new status"
                                                required
                                            />
