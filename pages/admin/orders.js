...
export default function Orders({ order }) {
    ...
    return (
        ...
        >
            ...
            <div className='container px-2 absolute right-0 mx-auto md:w-5/6 w-[100vw] min-h-screen'>
                <h1 className="font-bold  text-center text-xl my-4">My Orders</h1>

                <div className="relative flex overflow-x-auto min-h-screen shadow-md sm:rounded-lg table-auto">
                    {orders.length==0 && <h1>No Orders Found</h1>}
                    {orders.length>0 && orders.map((item)=>{return <div key={item._id}>
                         <div class="max-w-sm rounded  mx-2 overflow-hidden shadow-lg">
          ...
          index===Object.values(item.products).length-1?`${product.name} `:`${product.name} , `
                         ...
                         <div class="px-6 pt-4 pb-2">
                             <span class={`inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold  ${item.delivery=="unshipped"?'bg-yellow-200':'bg-green-500'} text-gray-700 mr-2 mb-2`}>Delivery : {item.delivery}</span>
                             <span class="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">Payment Status : {item.status}</span>
